...
struct ContentView: View {
    ...
    // Yêu cầu cấp quyền thông báo nếu lần đầu khởi động
    private func requestNotificationPermission() {
        ...
        if !launchedBefore {
            ...
            print("Ứng dụng đã được khởi động trước đó")
        }
    }
    
    // Lên lịch một Local Notification để test
    private func scheduleLocalNotification() {
        ...
        // Thiết lập thời gian kích hoạt sau 5 giây
...
// Delegate class để xử lý thông báo foreground
class NotificationDelegate: NSObject, UNUserNotificationCenterDelegate {
    func userNotificationCenter(_ center: UNUserNotificationCenter, willPresent notification: UNNotification, withCompletionHandler completionHandler: @escaping (UNNotificationPresentationOptions) -> Void) {
        completionHandler([.banner, .sound]) // Hiển thị banner và âm thanh khi ứng dụng đang mở
    }
}
